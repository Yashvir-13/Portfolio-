  ...
  if (source) {
    return sql`
      ...
      WHERE type = ${type}
        ...
        AND hidden = false
        AND source = ${source}
      ORDER BY featured DESC, display_order ASC NULLS LAST, date DESC NULLS LAST, created_at DESC
      LIMIT ${limit}
    `;
  }
...
export async function updateContent(id, data) {
  ...
  const rows = await sql`
    UPDATE content_items SET
      ...
      external_id = COALESCE(${data.external_id ?? null}, external_id),
...
export async function getExternalSources() {
  const sql = getDb();
  return sql`SELECT * FROM external_sources WHERE enabled = true ORDER BY name`;
}
...
export async function upsertExternalContent(data) {
  ...
  const rows = await sql`
    ...
    DO UPDATE SET
      ...
      date = EXCLUDED.date,
      excerpt = COALESCE(NULLIF(EXCLUDED.excerpt, ''), content_items.excerpt),
      body = COALESCE(NULLIF(EXCLUDED.body, ''), content_items.body),
      thumbnail = COALESCE(NULLIF(EXCLUDED.thumbnail, ''), content_items.thumbnail),
      hero_image = COALESCE(NULLIF(EXCLUDED.hero_image, ''), content_items.hero_image),
      canonical_url = COALESCE(NULLIF(EXCLUDED.canonical_url, ''), content_items.canonical_url),
      tags = CASE WHEN cardinality(EXCLUDED.tags) > 0 THEN EXCLUDED.tags ELSE content_items.tags END,
